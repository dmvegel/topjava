...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCyclesOptional(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        int calories = 0;
        List<UserMealWithExcess> resultExcessTrue = new ArrayList<>();
        List<UserMealWithExcess> resultExcessFalse = new ArrayList<>();

        for (UserMeal meal : meals) {
            if (TimeUtil.isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                calories += meal.getCalories();
                resultExcessTrue.add(new UserMealWithExcess(meal.getDateTime(), meal.getDescription(), meal.getCalories(), true));
                resultExcessFalse.add(new UserMealWithExcess(meal.getDateTime(), meal.getDescription(), meal.getCalories(), false));

            }
        }

        return calories > caloriesPerDay ? resultExcessTrue : resultExcessFalse;
    }
